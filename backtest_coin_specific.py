...
def main():
 ...
 for coin in SYMBOLS:
  ...
  for i in range(51,len(b)-25):
   sg=signals(f(b,i));o=outcome(b,i)
   if not o:continue
 ...
 payload={'version':'6.0.1','purpose':'COIN_SPECIFIC_RESEARCH','source':'BINANCE_SPOT','months':a.months,'train_fraction':.67,'test_fraction':.33,'horizon_hours':24,'costs':{'fee_per_side':FEE,'slippage_per_side':SLIP},'selection_rule':'TRAIN only: >=30 trades, PF>1, expectancy>0; then evaluate unchanged on TEST','results':allres}
