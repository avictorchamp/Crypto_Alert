...
H='https://data.binance.vision/data/spot/monthly/klines/{s}/1h/{s}-1h-{m}.zip';D='https://data.binance.vision/data/spot/monthly/klines/{s}/1d/{s}-1d-{m}.zip';FEE=.001;SLIP=.0005;HORIZON=24;RULES=('baseline','trend_pullback','support_rsi','reclaim','momentum')
def months(a,b):
 y,m=a.year,a.month
...
def stats(rows):
 ...
 r=[x[0] for x in rows];w=[x for x in r if x>0];loss=-sum(x for x in r if x<0);pf=sum(w)/loss if loss else None;return {'trades':len(r),'win_rate_pct':round(100*len(w)/len(r),2),'expectancy_pct':round(100*sum(r)/len(r),4),'profit_factor':round(pf,3) if pf is not None else None}
def main():
 ...
 for s in SYMBOLS:
  ...
  n=len(events);start_i=max(0,n//5);block=max(1,(n-start_i)//4);summary={}
  for mode in ('1H_ONLY','1D_BULL_FILTER'):
   by_rule={}
   for rule in RULES:
    folds=[]
    for f in range(4):
     lo=start_i+f*block;hi=start_i+(f+1)*block if f<3 else n;train=events[start_i:lo];test=events[lo:hi];allow=lambda e: mode=='1H_ONLY' or e[1]=='BULL';tr=[e[2] for e in train if allow(e) and e[3].get(rule)];te=[e[2] for e in test if allow(e) and e[3].get(rule)];folds.append({'train':stats(tr),'test':stats(te)})
    all_test=[]
    for f in range(4):
     lo=start_i+f*block;hi=start_i+(f+1)*block if f<3 else n;all_test += [e[2] for e in events[lo:hi] if (mode=='1H_ONLY' or e[1]=='BULL') and e[3].get(rule)]
    passed=sum(1 for z in folds if z['train']['trades']>=10 and z['test']['trades']>=10 and (z['train']['profit_factor'] or 0)>1 and (z['train']['expectancy_pct'] or -999)>0 and (z['test']['profit_factor'] or 0)>1 and (z['test']['expectancy_pct'] or -999)>0);overall=stats(all_test);by_rule[rule]={'folds':folds,'passed_folds':passed,'combined_test':overall,'robust':passed>=3 and overall['trades']>=50 and (overall['profit_factor'] or 0)>1.05 and (overall['expectancy_pct'] or -999)>0}
   good=[(v['combined_test']['expectancy_pct'],k) for k,v in by_rule.items() if v['robust']];good.sort(reverse=True);summary[mode]={'rules':by_rule,'selected':good[0][1] if good else None}
  result[s]={'candles_1h':len(h),'candles_1d':len(d),'events':n,'summary':summary}
 out={'version':'7.1.0','purpose':'MULTI_TIMEFRAME_WALK_FORWARD_RESEARCH','source':'BINANCE_DATA_ARCHIVE','months':a.months,'entry_interval':'1h','context_interval':'1d','horizon_hours':HORIZON,'costs':{'fee_per_side':FEE,'slippage_per_side':SLIP},'comparison':['1H_ONLY','1D_BULL_FILTER'],'robust_gate':'>=3/4 chronological folds, >=50 combined test trades, PF>1.05, positive expectancy','production_changed':False,'results':result};open('backtest_coin_specific_results.json','w').write(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
