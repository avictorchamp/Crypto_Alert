...
Uses Binance Vision monthly archives with daily fallback and accumulates
24h paper outcomes for historical OPEN signals.
"""
...
def close_ready(observations,h):
 ...
 for o in observations:
  ...
  exit_c=future[0]['c']; entry=float(o['price'])
  gross=(exit_c/entry-1)*100
  net=gross-200*FEE-200*SLIP
  o['exit_time_utc']=datetime.fromtimestamp(future[0]['t']/1000,timezone.utc).isoformat()
  o['exit_price']=exit_c
  o['paper_return_pct']=round(net,4)
  o['status']='CLOSED'
...
def main():
 ...
 data['summary']={'closed_trades':len(closed),'win_rate_pct':round(100*len(wins)/len(closed),2) if closed else None,'expectancy_pct':round(sum(closed)/len(closed),4) if closed else None,'profit_factor':round(sum(wins)/loss,3) if loss else None,'open_trades':sum(1 for x in data['observations'] if x.get('status')=='OPEN')}
