...
of a 24-month data set, with the preceding 12 months retained only for indicator
warm-up and 1D regime context. Research only; production is untouched.
"""
...
def main():
    ...
    for s in SYMBOLS:
        try:
            ...
            # Last 12 months split into four chronological 3-month windows.
            ...
            for w in range(4):
                a=lo + (span*w)//4
                b=lo + (span*(w+1))//4 if w<3 else hi+1
            ...
            for mode,rule in LOCKED[s].items():
                ...
                positive=sum(1 for x in fold if x['positive'])
